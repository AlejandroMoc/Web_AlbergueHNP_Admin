...
const banClient = async (id_usuario, id_cliente, notas_v) => {
  try {
    const ban = await db.none(`CALL veto_proc($1, $2, $3)`, [
      id_usuario,
      id_cliente,
      notas_v,
    ]);
...
const genWhereClause = (select_Filters, select_View, debtRange, dateRange) => {
  ...
  if (
    ...
    if (select_Filters.length !== 0) {
      i[0] = 1;
      filterConditions = select_Filters
        .map((filter) => {
          if (filter == 1 || filter == 2 || filter == 3 || filter == 6) {
            return `${filterColumnDB[filter - 1].column} = ${
              filterColumnDB[filter - 1].valdb
            }`;
          } else if (filter == 4) {
    ...
    if (i[0] == 1 && i[1] == 1 && i[2] == 1) {
      return (
        init + filterConditions + " AND " + filterDebt + " AND " + filterDate
      );
...
const getClientsByFilter = async (
  ...
  try {
    const whereClause = genWhereClause(
      select_Filters,
      select_View,
      debtRange,
      dateRange
    ...
    if (select_View == 10) {
      //console.log('ENTRA1')
      const clients = await db.any(
        "SELECT * FROM getClientsByFilterGeneral_func($1)",
        [whereClause]
      );
      //console.log('LO HIZO')
      //console.log(clients)
      ...
      const clients = await db.any(
        "SELECT * FROM getClientsByFilterHuesped_func($1)",
        [whereClause]
      );
      return clients;
    } else if (select_View == 8) {
      const clients = await db.any(
        "SELECT * FROM getClientsByFilterVisitaPrevia_func($1)",
        [whereClause]
      );
      return clients;
    } else if (select_View == 9) {
      const clients = await db.any(
        "SELECT * FROM getClientsByFilterServicios_func($1)",
        [whereClause]
      );
      return clients;
    }
