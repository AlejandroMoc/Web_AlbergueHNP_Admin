...
const getclienteInfoD = async (id_cliente) => {
  try {
    const clientInfo = await db.any(
      `SELECT cliente.*, paciente.*, area.nombre_a
        FROM cliente, paciente, area
        WHERE cliente.id_cliente = $1 
          AND cliente.carnet = paciente.carnet
          AND paciente.id_area = area.id_area;`,
      [id_cliente]
    );
    return clientInfo[0];
  } catch (error) {
...
const getHuespedInfo = async (id_cliente) => {
  try {
    const huespednfo = await db.any(
      `SELECT fecha_i, id_cama, fecha_s  FROM huesped WHERE id_cliente = $1;`,
      [id_cliente]
    );
    return huespednfo[0];
  } catch (error) {
...
const getDeudaCliente = async (id_cliente) => {
  try {
    const deudaCliente = await db.any(
      `SELECT calcular_deuda_cliente($1) AS DeudaCliente;`,
      [id_cliente]
    );
...
const getServicioEU = async (id_cliente) => {
  try {
    const servicioEU = await db.any(
      `SELECT * FROM calcular_suma_servicios_por_tipo($1);`,
      [id_cliente]
    );
    return servicioEU[0];
  } catch (error) {
...
const getVetado = async (id_cliente) => {
  try {
    const clientVeto = await db.any(
      `SELECT CASE WHEN EXISTS (
            SELECT 1 FROM vetado WHERE id_cliente = $1
        ) THEN 'true' ELSE 'false' END AS vetadoBool;;`,
      [id_cliente]
    );
    return clientVeto[0];
  } catch (error) {
