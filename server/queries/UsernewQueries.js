...
const registerEntradaUnica = async (
  id_usuario,
  carnet,
  id_area,
  nombre_p,
...
const getAllClientInfo = async (nombre_c, apellidos_c) => {
  ...
  try {
    const client = await db.any(
      `SELECT cliente.sexo, cliente.nivel_se, cliente.lugar_o, paciente.nombre_p, paciente.apellidos_p,
            cliente.carnet, paciente.id_area, cliente.notas_c FROM cliente LEFT JOIN paciente ON cliente.carnet = paciente.carnet
            WHERE nombre_c = $1 AND apellidos_c = $2`,
...
const getcarnet = async (carnet) => {
  try {
    const carnetExist = await db.any(
      `SELECT CASE WHEN EXISTS (
            SELECT 1 FROM paciente WHERE carnet = $1
        ) THEN 'true' ELSE 'false' END AS carnetExist;;`,
      [carnet]
    );
    return carnetExist;
  } catch (error) {
